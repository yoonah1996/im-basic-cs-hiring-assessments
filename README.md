...
3. MDN / Offical Document 이외에는 활용할 수 없습니다.
4. 이전에 풀었던 코드를 그대로 참고해서 문제를 푸시면 안됩니다. 본인이 참고했던 자료나 글들은 모두 각 문제에 코멘트로 남겨주세요. (링크들을 첨부해주시면 됩니다.)
5. 가능하면 랩탑만 사용하세요. 모니터를 사용을 원하시는 경우 화면을 반드시 복제(clone or mirroring)하세요. extended 화면은 사용할 수 없습니다. (프로젝트 문제는 필요 시 듀얼모니터 사용이 허용됩니다. )
6. 문제를 푸는 모든 기간 동안 화면 녹화가 진행 중이어야 합니다. 혹시라도 중간에 녹화가 되고 있지 않다면 즉시 재녹화를 해 주셔야 합니다.
7. 다른 사람이 제출한 내용을 절대로 참고하지 마세요.
8. Assessment git repository 를 fork & clone 하세요.
9. `student.json`에 수강생분의 기수 이름을 입력해주세요.
10. 객관식 문제 & 코딩 문제는 마감시간까지 반드시 `npm run submit` 및 제출하셔야 합니다. 시험 시간이 종료되기 전에 `npm run submit` 할 시간을 남겨주세요. 또한 제출하기 전에 결과 값을 업데이트 하기 위해 `npm run test` 를 다시 실행해 주시기 바랍니다.
11. 시험이 일찍 종료되신 분들은 제출하신 뒤에 일찍 끝내셔도 됩니다.
13. 반드시 모든 문제를 조금이라도 풀려고 시도하시길 바랍니다. 코드를 작성할 시간이 부족하더라도 코멘트를 남겨서 어떤 식으로 풀어가려고 했는지에 대해 작성하시기 바랍니다. 아예 한 문제에 대해서 아무것도 하지않고 그대로 두는것보다는 pseudocode 나 comment 를 작성하는 것이 추후 이의제기(claim)이 필요한 경우 부분 평가를 받을 수도 있습니다.
